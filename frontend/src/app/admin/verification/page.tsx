...
export default function VerificationAdminPage() {
  ...
  const fetchFrames = useCallback(async () => {
    try {
      ...
      console.error("[HITL] Error fetching verification frames:", err);
    }
  ...
  // Render HTML5 Canvas with Bounding Boxes
  useEffect(() => {
    ...
    img.src = imgUrl;
    img.onload = () => {
      // Set canvas dimension matching container max width (e.g. 720px width)
      const maxW = 720;
      ...
      // Draw Model Prediction Bounding Boxes (Dashed lines / Orange)
      ...
      // Draw Human Annotation Bounding Boxes (Solid Cyan/Purple lines)
      ctx.setLineDash([]);
  ...
  // Canvas Mouse Event Handlers for Interactive Box Drawing
  ...
  const handleMouseUp = () => {
    ...
    // Filter tiny accidental clicks (< 10px)
    if (w > 10 && h > 10) {
  ...
  // Decision Handler (Approve / Reject / Correct)
  const handleDecision = async (statusDecision: "approved" | "rejected" | "corrected") => {
    ...
    try {
      ...
      // Refresh frame list and advance to next frame
      fetchFrames();
      ...
      console.error("[HITL] Error updating annotation decision:", err);
    }
  };
  ...
  const handleIngest = async () => {
    ...
    try {
      ...
      console.error("[HITL] Ingestion failed:", err);
    } finally {
      ...
      <header className={styles.header}>
        <div className={styles.brand}>
          ...
          <div>
            ...
            <div className={styles.subtitle}>HUMAN-IN-THE-LOOP (HITL) VERIFICATION DASHBOARD</div>
          </div>
        </div>
        ...
        <nav className={styles.navLinks}>
          ...
          <Link href="/admin/verification" className={`${styles.navBtn} ${styles.navBtnActive}`}>
            <CheckSquare size={14} /> HITL VERIFICATION
          </Link>
        </nav>
